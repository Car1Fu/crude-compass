...
(()=>{
  ...
  const impactCls=v=>v>=80?'impact-high':v>=50?'impact-mid':'impact-low';
  ...
  function render(){
    ...
    sec.innerHTML=MODS.map(m=>{
      if(m.key==="HIGHLIGHT"){
        const sideRows=hlSide.map(x=>`<div class="hl-item" data-id="${x.id}"><div class="hl-src">${esc(x.source)}</div><p class="hl-title">${esc(x.title)}</p><div class="hl-meta"><span>${fD(x.ts)} ${fT(x.ts)}</span>${impactSpan(x.impact)}</div></div>`).join("")||`<div class="hl-item"><p class="hl-title" style="color:rgba(255,255,255,.4)">（暂无）</p></div>`;
        return`<div class="mn-section" id="sec-HIGHLIGHT"><div class="mn-section-head"><h3>${esc(m.name)}</h3><div class="mn-section-meta"><span>${H.length} 条</span><span class="mn-sep"></span><button class="mn-btn primary" data-vm data-key="HIGHLIGHT" style="padding:4px 10px;font-size:11px;border-radius:8px;">查看更多</button></div></div><div class="mn-body mn-body-hl">${fc(hlTop)}<div class="mn-panel">${sideRows}</div></div></div>`;
      }
      const la=filtered.filter(x=>x.theme===m.key).sort((a,b)=>b.score-a.score);
      const rows=la.map(x=>`<div class="mn-news-row" data-id="${x.id}"><div class="nr-src">${esc(x.source)}</div><div class="nr-title">${esc(x.title)}</div><div class="nr-meta"><span>${fD(x.ts)} ${fT(x.ts)}</span>${impactSpan(x.impact)}</div></div>`).join("")||`<div class="mn-news-row"><div class="nr-title" style="color:rgba(255,255,255,.4)">当前搜索下暂无</div></div>`;
      return`<div class="mn-section" id="sec-${m.key}"><div class="mn-section-head"><h3>${esc(m.name)}</h3><div class="mn-section-meta"><span>${la.length} 条</span><span class="mn-sep"></span><button class="mn-btn primary" data-vm data-key="${m.key}" style="padding:4px 10px;font-size:11px;border-radius:8px;">更多</button></div></div><div class="mn-body"><div class="mn-news-list">${rows}</div></div></div>`;
    }).join("");
  ...
  const ref=document.getElementById("mnRefresh");if(ref)ref.addEventListener("click",()=>{S.items=gen();render();});
  render();
  renderQuickNav();
})();
